...
export function ProjectShowcase({ name, description, link, image, video, technologies = [] }: ProjectProps) {
    ...
    >
      <MorphingDialogTrigger>
        <div className="group relative overflow-hidden rounded-xl bg-zinc-50/40 p-1 ring-1 ring-zinc-200/50 ring-inset transition-all duration-300 hover:shadow-md dark:bg-zinc-950/40 dark:ring-zinc-800/50">
          {image ? (
            <Image src={image} alt="preview-img" width={300} height={200} className="aspect-video w-full cursor-zoom-in rounded-xl" />
          ) : (
      ...
      <MorphingDialogContainer>
        <MorphingDialogContent className="relative max-h-[90vh] w-[90vw] max-w-5xl overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-zinc-900">
          ...
          <MorphingDialogClose className="absolute right-4 top-4 rounded-full bg-white p-1.5 text-zinc-500 shadow-md transition-colors hover:text-zinc-900 dark:bg-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-100">
